				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="座位数" prop="zuoweishu">
							<el-input class="list_inp" v-model="form.zuoweishu" placeholder="座位数"
								 type="text" 								:readonly="!isAdd||disabledForm.zuoweishu?true:false" />
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.xiangqing = res.data.data.xiangqing?(res.data.data.xiangqing.replace(reg,'../../../cl46724236/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='zuoweishu'){
					form.value.zuoweishu = row[x];
					disabledForm.value.zuoweishu = true;
					continue;
				}
				if(x=='meixiaoshijia'){
