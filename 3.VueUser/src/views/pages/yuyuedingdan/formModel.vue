	...
	<div class="app-contain" :style='{"padding":"20px 20px","margin":"20px auto 40px","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"1200px","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">自习室位置</div>
					<div  class="info_text" >{{detail.zixishiweizhi}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">座位号</div>
					<div  class="info_text" >{{detail.zuoweihao}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">每小时价</div>
					<div  class="info_text" >{{detail.meixiaoshijia}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">离开时间</div>
					<div  class="info_text" >{{detail.likaishijian}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">预约时长</div>
					<div  class="info_text" >{{detail.yuyueshizhang}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'取消订单')" @click="quxiaodingdanonAcross('取消订单','quxiaodingdan','是','','')" type="warning">取消订单</el-button>
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'评价')" @click="pingjiaxinxionAcross('评价','pingjiaxinxi','否','','')" type="warning">评价</el-button>
					<el-button class="approval_btn" v-if="btnAuth('yuyuedingdan','审核')" type="warning" @click="approvalClick()">审核</el-button>
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('yuyuedingdan','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('yuyuedingdan','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('yuyuedingdan','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
...
<script setup>
	...
	const quxiaodingdanonAcross = (btnType,table,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
