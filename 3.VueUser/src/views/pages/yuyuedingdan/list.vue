	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0","margin":"0 auto","overflow":"hidden","borderRadius":"4px","background":"#fff","width":"1200px","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('yuyuedingdan','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="自习室名称" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.zixishimingcheng}}
							</template>
						</el-table-column>
						<el-table-column label="自习室位置" :resizable='true' align="left" header-align="left">
							<template #default="scope">
						...
						<el-table-column label="自习室类型" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.zixishileixing}}
							</template>
						</el-table-column>
						<el-table-column label="座位号" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.zuoweihao}}
							</template>
						</el-table-column>
						<el-table-column label="每小时价" :resizable='true' align="left" header-align="left">
							<template #default="scope">
						...
						<el-table-column label="离开时间" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.likaishijian}}
							</template>
						</el-table-column>
						<el-table-column label="预约时长" :resizable='true' align="left" header-align="left">
							<template #default="scope">
						...
						<el-table-column label="是否支付" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								...
								<el-button v-if="scope.row.ispay!='已支付' && btnAuth('yuyuedingdan','支付')" type="text" size="small" @click.stop="payClick(scope.row)">支付</el-button>
							</template>
						</el-table-column>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/yuyuedingdanAdd')
	}
	//判断是否从个人中心跳转
	...
	const tableDetailClick = (row) => {
		router.push('yuyuedingdanDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
